...
| 1.8 Transcript | done | Doc-projection source, virtualized, markdown + syntax highlight, tool folding (ToolGroup/ToolChip), input/error chips, stick-to-bottom band, MessageRail minimap (hover preview, hidden < 48rem), user-bubble attachment thumbnails (112×80, read-back from owning device, 2s→15s retry ladder, seeded cache, click-to-expand lightbox). Live-tail group header names the current activity ("Running cargo test", "Thinking…") instead of the count summary; thoughts report UI-measured duration ("Thought for 12s"); detail-less chips pulse while running and cross on failure; thought bodies render their FULL text in a max-height scrollbox that follows the stream tail (wheel releases, end re-engages) instead of a counted-tail truncation; overflowing bodies get horizontal padding and the model picker's floating scrollbar (inset track, 3px↔5px thumb, drag / click-to-jump, wheel contained inside the box). |
