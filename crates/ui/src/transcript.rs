...
/// thought's markdown flattened into styled lines ([`thought_lines`]) as a
/// bounded SCROLLBOX detail (waku's reasoning peek) — the full text, never
/// truncated; overflow scrolls and follows the stream tail while live.
/// Analytic heights still hold (the box clamps at [`THOUGHT_MAX_H`]), so
/// the group's fold tween keeps working. `live` = the part is still
/// streaming (chip defaults open).
fn thought_item(tree: &BlockTree, live: bool) -> ToolItem {
    ...
    ToolItem {
        ...
        is_error: false,
        resolved: !live,
        detail: (!lines.is_empty()).then(|| Arc::new(ToolDetail::Thought { lines })),
        invocation: None,
        output_ref: None,
        output_bytes: None,
        diff_ref: None,
...
pub enum ToolDetail {
    ...
    /// A REASONING part's markdown flattened into wrapped STYLED lines
    /// ([`thought_lines`]) — one fixed-height row each, so heights stay
    /// analytic like `Output`, while inline markers render as real styling.
    /// Deliberately NEVER truncated: the body renders inside a bounded
    /// scrollbox that follows the stream tail (waku's reasoning peek), so
    /// length costs scroll, not lost context.
    Thought { lines: Vec<Vec<InlineRun>> },
    ...
    for t in tools {
        ...
        match t.detail.as_deref() {
            ...
            Some(ToolDetail::Thought { lines }) => {
                // Hash run BYTES plus style bits: thought text GROWS while
                // it streams, and a live mend can restyle runs without
                // changing the flattened length — either must re-splice.
                acc.push(4);
                acc.extend_from_slice(&(lines.len() as u32).to_le_bytes());
                for line in lines {
                    for run in line {
...
impl Transcript {
    ...
    fn thought_body(
        ...
        column = column.children(
            lines[first..last]
                .iter()
                .map(|line| styled_thought_row(line, theme)),
        );
        if last < lines.len() {
            column = column.child(
                div()
...
fn detail_body(
    ...
    match detail {
        ...
        // the same styled rows without the scrollbox.
        ToolDetail::Thought { lines } => body
            .py(px(6.0))
            .text_size(px(12.0))
            .children(lines.iter().map(|line| styled_thought_row(line, theme)))
            .into_any_element(),
    }
}
...
/// One fixed-height styled markdown row of a thought body — the scrollbox
/// window and the fallback detail block render the same rows. Blank
/// separator lines come back as an empty row.
fn styled_thought_row(line: &[InlineRun], theme: &Theme) -> gpui::Div {
    let row = div()
        .h(px(OUTPUT_LINE_HEIGHT))
        .w_full()
        .min_w_0()
...
mod tests {
    ...
    fn reasoning_joins_the_tool_group_accordion() {
        ...
        // Thought chips carry their FULL markdown text as a scrollbox detail
        // (never truncated); the clamped height keeps the fold tween
        // analytic.
        assert!(matches!(
    ...
    fn thought_detail_keeps_every_line_in_a_scrollbox() {
        let long = (0..60)
            ...
            .collect::<Vec<_>>()
            .join(" ");
        let item = thought_item(&parse_full(&long), false);
        ...
        let short = thought_item(&parse_full("quick thought"), false);
    ...
    fn thought_fingerprint_tracks_content_not_just_shape() {
        ...
        let a = thought_item(&parse_full("abcdef ghijkl"), false);
        let b = thought_item(&parse_full("ghijkl abcdef"), false);
        assert_ne!(
            tool_fingerprint(std::slice::from_ref(&a), false),
            tool_fingerprint(std::slice::from_ref(&b), false)
        );
        // Growth keeps tripping it, as before.
        let grown = thought_item(
            &parse_full("abcdef ghijkl and then more prose arrived"),
            false,
        );
        assert_ne!(
            tool_fingerprint(std::slice::from_ref(&a), false),
